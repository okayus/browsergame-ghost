...
- [x] 17.3 (P) ローディング画面コンポーネントを実装する
  - セーブデータ読み込み中のローディングアニメーション表示
  - 読み込み状態のテキスト表示
  - Tailwind CSSアニメーションユーティリティの活用
  - _Requirements: 12.5_

- [x] 17.4 アプリのルートコンポーネントを認証フローに対応させる
...
- [ ] 19. パーティ画面表示機能実装
- [x] 19.1 (P) ゴースト簡易表示カードコンポーネントを実装する
  - ゴーストの名前（種族名またはニックネーム）を表示
  - レベル表示（Lv. X形式）
  - 現在HP/最大HPをHPバーと数値で表示
  - 選択状態の視覚的フィードバック（ボーダー色変更など）
  - クリック/タップでの選択インタラクション
  - _Requirements: 14.2, 14.3_

- [x] 19.2 (P) ゴースト詳細パネルコンポーネントを実装する
  - ゴーストのタイプをアイコンまたはラベルで表示
  - 能力値（HP、攻撃、防御、素早さ）を数値またはゲージで表示
  - 覚えている技一覧（最大4つ）を技名とタイプで表示
  - 技のPP残量表示
  - 閉じるボタンまたはEscapeキーで一覧に戻る
  - _Requirements: 14.4_

- [x] 19.3 パーティ画面コンポーネントを実装する
  - パーティ内ゴースト（最大6体）の一覧表示
  - GhostSummaryCardを使用した各ゴーストの簡易情報表示
  - 上下キー（W/S、矢印キー）でのゴースト選択移動
  - Enterキーでゴースト詳細パネルを開く
  - Escapeキーでメニュー画面に戻る
  - ゴースト0体の場合の空状態表示
  - _Requirements: 14.1, 14.2, 14.5, 14.6_

- [x] 19.4 メニューからパーティ画面への遷移を実装する
  - MenuScreenの「パーティ」選択時にsetScreen("party")を呼び出し
  - App.tsxでgameState.currentScreen === "party"時にPartyScreenを表示
  - パーティ画面へのキー入力転送
  - パーティ画面からメニュー画面への戻り処理
  - _Requirements: 14.1, 14.6_

- [ ] 19.5* パーティ画面のテストを実装する
  - GhostSummaryCardのレンダリングテスト
  - GhostDetailPanelのレンダリングテスト
  - PartyScreenのキーボード操作テスト
  - 画面遷移テスト（メニュー→パーティ→メニュー）
  - _Requirements: 14.1, 14.2, 14.3, 14.4, 14.5, 14.6_

- [x] 20. バトル終了後のパーティHP同期機能
- [x] 20.1 バトル終了時HP同期フックを実装する
  - バトル状態からパーティHP更新ロジックを作成
  - 勝利/逃走/捕獲時は戦闘中のHPをパーティに反映
  - 敗北時はパーティ全員のHPを最大HPまで回復
  - useGameStateのupdatePartyGhostを呼び出してHP更新
  - セーブキューへの追加処理を実装
  - _Requirements: 15.1, 15.2, 15.3, 15.4, 15.5_

- [x] 20.2 バトル終了ハンドラーにHP同期を統合する
  - App.tsxのバトル終了処理にuseBattleEndSyncを組み込み
  - resetBattle()呼び出し前にsyncPartyHpを実行
  - バトル結果表示後のタイミングで同期処理を発火
  - 全バトル終了パス（勝利、敗北、逃走、捕獲）で統一的に動作
  - _Requirements: 15.1, 15.2, 15.3, 15.4_

- [x] 21. バトル中アイテム使用機能
- [x] 21.1 アイテム使用ロジックフックを実装する
  - 回復アイテム使用時のHP回復処理を作成
  - 捕獲アイテム使用時のボーナス計算処理を作成
  - インベントリからのアイテム消費処理を実装
...
- [x] 21.2 アイテムコマンドをバトル画面に接続する
  ...
  - カテゴリ（回復系、捕獲系）ごとのアイテム表示
  - _Requirements: 16.1, 16.2_

- [x] 21.3 回復アイテム使用フローを実装する
  - 回復アイテム選択時にプレイヤーゴーストのHPを回復
  - ターン消費してバトルを継続
  - 回復メッセージを表示
  - アイテム使用後はコマンド選択に戻らず敵のターンへ
  - _Requirements: 16.3, 16.4_

- [x] 21.4 捕獲アイテム使用フローを実装する
  - 捕獲アイテム選択時にボーナス計算
  - 既存のexecutePlayerActionにitemBonusを渡して捕獲処理を実行
  - 捕獲成功/失敗の結果表示
  - アイテム消費とインベントリ更新
  - _Requirements: 16.5, 16.6_

- [x] 21.5 アイテム選択画面のキャンセル処理を実装する
  - Escapeキーまたはキャンセルボタンでコマンド選択に戻る
  - 所持数0のアイテムは選択不可として表示
  - アイテムがない場合のメッセージ表示
  - _Requirements: 16.8, 16.9_

- [x] 22. 手動セーブ機能
- [x] 22.1 セーブフィードバックコンポーネントを実装する
  - セーブ中のローディングインジケーター表示
  - セーブ成功時のメッセージ表示（2秒後に自動消去）
  - セーブ失敗時のエラーメッセージとリトライボタン表示
  - ManualSaveStatus型に基づいた状態管理
  - _Requirements: 17.2, 17.3, 17.4_

- [x] 22.2 メニュー画面にセーブ機能を統合する
  - handleMenuSelectの「save」ケースを完全実装
  - useSaveDataMutationを使用したセーブ実行
  - セーブ中はメニュー操作を無効化
  - セーブ完了後もメニュー画面を維持
  - SaveFeedbackコンポーネントの表示統合
  - _Requirements: 17.1, 17.5_

- [x] 23. 追加機能の統合テスト
- [x] 23.1* バトル終了HP同期のテストを実装する
  - 勝利時のHP同期テスト
  - 敗北時の全回復テスト
  - 逃走/捕獲時のHP同期テスト
  - セーブキュー追加の確認テスト
...
| 13.1-13.5 | 17.5 |
| 14.1-14.6 | 19.1, 19.2, 19.3, 19.4, 19.5 |
| 15.1-15.5 | 20.1, 20.2, 23.1 |
| 16.1-16.9 | 21.1, 21.2, 21.3, 21.4, 21.5, 23.2 |
| 17.1-17.5 | 22.1, 22.2, 23.3 |
