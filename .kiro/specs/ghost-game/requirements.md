...
### Requirement 15: バトル終了後のパーティHP同期

**Objective:** As a プレイヤー, I want バトル終了後に受けたダメージがパーティに反映されることで, so that バトルに緊張感があり回復アイテムの価値が生まれる

#### Acceptance Criteria

1. When バトルが勝利で終了した時, the ゲームシステム shall バトル中のプレイヤーゴーストの現在HPをパーティデータに反映する
2. When バトルが逃走成功で終了した時, the ゲームシステム shall バトル中のプレイヤーゴーストの現在HPをパーティデータに反映する
3. When バトルが捕獲成功で終了した時, the ゲームシステム shall バトル中のプレイヤーゴーストの現在HPをパーティデータに反映する
4. When バトルが敗北で終了した時, the ゲームシステム shall パーティ全員のHPを最大HPまで回復する
5. When パーティHPが更新された時, the ゲームシステム shall セーブデータの更新をキューに追加する

### Requirement 16: バトル中アイテム使用

**Objective:** As a プレイヤー, I want バトル中にアイテムを使用したい, so that 回復や捕獲を戦略的に行える

#### Acceptance Criteria

1. When プレイヤーがバトル中に「アイテム」コマンドを選択した時, the ゲームシステム shall アイテム選択画面に遷移する
2. When アイテム選択画面を表示する時, the ゲームシステム shall 所持アイテムをカテゴリ（回復系、捕獲系）ごとに表示する
3. When プレイヤーが回復アイテムを選択した時, the ゲームシステム shall 対象ゴーストのHPを回復効果値分回復させる
4. When 回復アイテムを使用した時, the ゲームシステム shall ターンを消費してバトルを継続する
5. When プレイヤーが捕獲アイテムを選択した時, the ゲームシステム shall 捕獲処理を実行する
6. When 捕獲アイテムを使用した時, the ゲームシステム shall アイテムの効果値を捕獲成功率に加算する
7. When アイテムを使用した時, the ゲームシステム shall 所持アイテムの数量を1減少させる
8. When アイテム選択画面でキャンセル操作をした時, the ゲームシステム shall コマンド選択画面に戻る
9. If 所持数が0のアイテムがある時, the ゲームシステム shall そのアイテムを選択不可として表示する

### Requirement 17: 手動セーブ機能

**Objective:** As a プレイヤー, I want メニューから手動でセーブしたい, so that 重要な進行状況を確実に保存できる

#### Acceptance Criteria

1. When プレイヤーがメニューで「セーブ」を選択した時, the ゲームシステム shall 現在のゲーム状態（位置、パーティ、インベントリ）をバックエンドに保存する
2. While セーブ処理中の時, the ゲームシステム shall セーブ中であることを示すインジケーターを表示する
3. When セーブが成功した時, the ゲームシステム shall 「セーブしました」というメッセージを表示する
4. If セーブが失敗した時, the ゲームシステム shall エラーメッセージを表示しリトライオプションを提供する
5. The ゲームシステム shall セーブ完了後もメニュー画面を維持する
